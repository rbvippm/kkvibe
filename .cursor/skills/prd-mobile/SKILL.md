...
description: 移动端 H5 / 代理端 PRD 标注规范；用于移动端原型、PRD、标注、Mh5SpecAnnot、MobilePrdSpec、buildMobilePrdSections，区别于 PC 的 WfSpecAnnot 与 items[] 简版条目。后续移动端 PRD 不增加注N 数量，放到对应的功能位置即可，名字默认叫标注。
---
...
- 代理端 / H5 / `/mobile/*` 页面需要加 PRD「标注」。
- 用户提到 `移动端标注`、`移动 PRD`、`H5 PRD`、`代理端 PRD`、`Mh5SpecAnnot`、`MobilePrdSpec`、`标注`。
...
- PC 后台：使用 `WfSpecAnnot` 和 PC PRD 规范，不使用本 Skill。PC 仍用「注N」，不要把移动端「标注」套到 PC。
- 禁止在移动端使用 PC 的 `WfSpecAnnot` 或 `items: string[]` 简版标注结构。
- 禁止将移动端 `Mh5SpecAnnot`、`MobilePrdSpec` 或六大结构套到 PC 后台页面。

## 核心产物

- 标注组件：`src/components/mobile/Mh5SpecAnnot.vue`（触发器默认文案「标注」，不展示注1 / 注2）
- PRD 类型：`src/constants/mobilePrdSpec.ts` → `MobilePrdSpec`
- 组装函数：`buildMobilePrdSections()`
- 大类序号：`MOBILE_PRD_SECTION_ORDINALS`（一至六，仅用于浮层六大结构，不是页面上的注N）
- 规范全文：`docs/H5移动端设计规范.md`
- 常量示例：`src/constants/livePipSpec.ts`
- 页面示例：`src/views/mobile/MobilePipSettingsView.vue`
...
- **三、视觉表现** `visual`：只写业务 UI 的布局、组件状态、动效；禁止写「标注」、标注入口、PRD 浮层本身。使用 `buildMobilePrdSections()` 时会自动追加「颜色、字号以实际设计稿为准。」。
...
3. 在 `src/constants/{module}Spec.ts` 定义一个或多个 `MobilePrdSpec` 常量；**不要**再写 `{MODULE}_SPEC_ANNOT_NO`，**不要**给 `spec` 加递增编号。
4. 使用 `buildMobilePrdSections({ logic, interaction, visual, data, exception, routing })` 填满六大结构。
5. 把 `Mh5SpecAnnot` **放到对应的功能位置**（标题旁、该行 / 该区块旁），不要为了编号堆在顶栏。
6. 若页面或功能已有标注，改动原型时同步更新对应 `MobilePrdSpec`；改动 `MobilePrdSpec` 时同步检查并调整页面原型。
7. 完成后检查六大结构、页面文案、Mock 数据、异常状态、路由说明和标注位置是否一致。
...
- 后续移动端 PRD 不需要增加数量，放到对应的功能位置即可，然后名字默认叫标注。
- 触发器文案固定为「标注」，由 `Mh5SpecAnnot` 默认渲染；不要写「注1」「注2」，不要维护 `SPEC_ANNOT_NO`。
- 同一页面可以有多条「标注」，靠放置位置区分功能，不靠数字区分。
- 一条「标注」只描述一个明确功能或流程；不要把多个无关功能塞进同一条。
- 不要把「标注」放进会触发主流程的按钮内部。
- PRD 浮层内：六大块标题为「一、功能逻辑」至「六、关联与跳转」；每块细项为「1. 2. 3.」。由 `Mh5SpecAnnot` 根据 `sections` 顺序和 `lines` 下标自动加序号，撰写 `*Spec.ts` 时只写正文。
- 历史页面若仍带 `spec.no` / 「注N」，改该页 PRD 时改为「标注」并挪到对应功能旁；不要为旧页继续递增编号。
...
- 视觉表现只描述业务 UI；不要描述「标注」触发器、PRD 浮层、遮罩或标注组件自身。
...
- 禁止新增「注N」编号，禁止 `{MODULE}_SPEC_ANNOT_NO`。
- 禁止把「标注」放在会触发主流程的按钮内部。
