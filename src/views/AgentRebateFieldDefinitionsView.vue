...
import {
  AGENT_REBATE_FIELD_DEF_FORMULAS,
  AGENT_REBATE_FIELD_DEF_META,
  AGENT_REBATE_FIELD_DEF_MODULES,
} from '../constants/agentRebateFieldDefinitions'
...
<template>
  <div class="agent-field-def agent-field-def--rebate">
    <header class="agent-field-def__header">
      <div class="agent-field-def__header-inner">
        ...
        <p class="agent-field-def__eyebrow">{{ AGENT_REBATE_FIELD_DEF_META.subtitle }}</p>
        <h1 class="agent-field-def__title">{{ AGENT_REBATE_FIELD_DEF_META.title }}</h1>
        <div class="agent-field-def__nav">
          <a
            v-for="mod in AGENT_REBATE_FIELD_DEF_MODULES"
            :key="mod.id"
    ...
    <main class="agent-field-def__main">
      ...
      >
        ...
        <div class="agent-field-def__table-wrap">
          <table class="agent-field-def__table">
            <thead>
              <tr>
                <th v-for="col in AGENT_REBATE_FIELD_DEF_META.columns" :key="col">{{ col }}</th>
              </tr>
            </thead>
            <tbody>
      ...
      <section class="agent-field-def__section">
        ...
        <ol class="agent-field-def__formulas">
          <li v-for="(item, index) in AGENT_REBATE_FIELD_DEF_FORMULAS" :key="index">{{ item }}</li>
        </ol>
      </section>
    </main>
  </div>
