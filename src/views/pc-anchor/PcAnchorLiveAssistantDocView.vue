...
import {
  LIVE_ANCHOR_ASSISTANT_BACKGROUND,
  LIVE_ANCHOR_ASSISTANT_FEATURE_LIST,
  LIVE_ANCHOR_ASSISTANT_GOALS,
  LIVE_ANCHOR_ASSISTANT_META,
} from '../../constants/liveAnchorAssistantSpec'
  ...
  <div class="pc-wireframe-page wf-doc-page">
    ...
    <section class="wf-block">
      <header class="wf-doc-page__header">
        <p class="wf-doc-page__module">所属模块：{{ LIVE_ANCHOR_ASSISTANT_META.module }}</p>
        <h2 class="wf-doc-page__title">需求名称：{{ LIVE_ANCHOR_ASSISTANT_META.title }}</h2>
        <p class="wf-doc-page__meta">
          PRD 版本：{{ LIVE_ANCHOR_ASSISTANT_META.prdVersion }} · 最近更新：{{ LIVE_ANCHOR_ASSISTANT_META.updatedAt }}
        </p>
        <p class="wf-doc-page__hint">{{ PRD_DIMENSION_HINT }}</p>
        <RouterLink :to="{ name: 'pca-live-assistant' }" class="wf-doc-page__back">
          ← 返回主播控制台
        </RouterLink>
      </header>
      ...
      <div v-if="activeTab === 'summary'" class="wf-doc-page__summary">
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in LIVE_ANCHOR_ASSISTANT_BACKGROUND" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in LIVE_ANCHOR_ASSISTANT_GOALS" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <div class="wf-table-wrap">
            <table class="wf-table">
              ...
              <tbody>
                <tr v-if="!LIVE_ANCHOR_ASSISTANT_FEATURE_LIST.length">
                  <td class="wf-td wf-td--empty" colspan="4">暂无功能清单，待梳理标注后补充。</td>
                </tr>
                <tr v-for="row in LIVE_ANCHOR_ASSISTANT_FEATURE_LIST" :key="row.id">
      ...
      <div v-else class="wf-doc-page__features">
        <p v-if="!LIVE_ANCHOR_ASSISTANT_FEATURE_LIST.length" class="wf-muted">暂无已标注功能，待梳理后补充。</p>
        <article
          v-for="row in LIVE_ANCHOR_ASSISTANT_FEATURE_LIST"
          :key="row.id"
