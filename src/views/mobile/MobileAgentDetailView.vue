...
function resolveAgentDetailTab(raw: unknown): AgentDetailTab | null {
  const key = String(raw ?? '')
  if (key === 'wallet' || key === 'credit' || key === 'profit' || key === 'game' || key === 'login') {
    return key
  }
  return null
}

const activeTab = ref<AgentDetailTab>(resolveAgentDetailTab(route.query.tab) ?? 'wallet')
...
const profitEarnDetailsExpanded = ref(false)
...
watch(
  () => route.query.tab,
  (tab) => {
    const next = resolveAgentDetailTab(tab)
...
watch(currency, () => {
  ...
  profitEarnDetailsExpanded.value = false
...
function toggleProfitEarnDetails() {
  profitEarnDetailsExpanded.value = !profitEarnDetailsExpanded.value
}
...
function goCreditStatement() {
  ...
  router.push({
    ...
    query: {
      currency: creditCurrency.value,
      from: 'agent-detail',
      keyword: getAgentDisplayName(agent.value),
      lane: 'downstream',
    },
  })
}
...
function settleAmountClass(positive?: boolean) {
  if (positive === true) return 'mh5-agent-detail-wallet__value--positive'
  if (positive === false) return 'mh5-agent-detail-wallet__value--negative'
  return 'mh5-agent-detail-credit-settle__value--neutral'
}
...
<template>
  <div class="mh5-agent-detail-page" @click="creditCurrencyMenuOpen = false">
    ...
    <main v-if="agent" class="mh5-agent-detail-main">
      ...
      <template v-else-if="activeTab === 'credit'">
        ...
        >
          ...
          >
            <p class="mh5-agent-detail-credit-settle__receivable-label">我方应收下级</p>
          ...
          <div class="mh5-agent-detail-credit-settle__timeline">
            ...
            >
              ...
              <span :class="settleAmountClass(item.positive)">{{ item.value }}</span>
            </div>
      ...
      <template v-else-if="activeTab === 'profit'">
        <section
          ...
          :class="{
            'mh5-agent-my-profit-main mh5-agent-my-profit-main--rebate': useProfitSections,
          }"
        ...
        >
          ...
          <section
            ...
            class="mh5-agent-detail-wallet mh5-agent-detail-profit-summary"
          >
            ...
            >
              ...
              >
                <button
                  type="button"
                  class="mh5-agent-my-profit-table__cell mh5-agent-my-profit-table__cell--name mh5-agent-my-profit-table__name-with-chevron mh5-agent-my-profit-table__expand-trigger"
                  :aria-expanded="profitEarnDetailsExpanded"
                  aria-label="展开或收起代理赚水细项"
                  @click="toggleProfitEarnDetails"
                >
                  <span>{{ profitEarnSection.total.label }}</span>
                  <span
                    class="mh5-agent-my-profit-table__chevron"
                    :class="{ 'mh5-agent-my-profit-table__chevron--open': profitEarnDetailsExpanded }"
                    aria-hidden="true"
                  >
                    ▾
                  </span>
              ...
              <Transition name="mh5-agent-my-profit-expand">
                <div v-if="profitEarnDetailsExpanded" class="mh5-agent-my-profit-table__details">
                  <button
                    v-for="(row, index) in profitEarnSection.rows"
                    :key="row.key"
                    type="button"
                    class="mh5-agent-my-profit-table__row"
                    :class="{ 'mh5-agent-my-profit-table__row--alt': index % 2 === 1 }"
                    :aria-label="`查看${row.label}赚水`"
                    @click="openProfitDialog(row.key)"
                  >
                    <span class="mh5-agent-my-profit-table__cell mh5-agent-my-profit-table__cell--name">
                      {{ row.label }}
