...
import {
  LIVE_ANCHOR_AUDIT_BACKGROUND,
  LIVE_ANCHOR_AUDIT_FEATURE_LIST,
  LIVE_ANCHOR_AUDIT_GOALS,
  LIVE_ANCHOR_AUDIT_META,
} from '../../constants/liveAnchorAuditSpec'
  ...
  <div class="pc-wireframe-page wf-doc-page">
    ...
    <section class="wf-block">
      <header class="wf-doc-page__header">
        <p class="wf-doc-page__module">所属模块：{{ LIVE_ANCHOR_AUDIT_META.module }}</p>
        <h2 class="wf-doc-page__title">需求名称：{{ LIVE_ANCHOR_AUDIT_META.title }}</h2>
        <p class="wf-doc-page__meta">
          PRD 版本：{{ LIVE_ANCHOR_AUDIT_META.prdVersion }} · 最近更新：{{ LIVE_ANCHOR_AUDIT_META.updatedAt }}
        </p>
        <p class="wf-doc-page__hint">{{ PRD_DIMENSION_HINT }}</p>
        <RouterLink :to="{ name: 'pc-live-anchor-audit' }" class="wf-doc-page__back">
          ← 返回主播审核列表
        </RouterLink>
      </header>
      ...
      <div v-if="activeTab === 'summary'" class="wf-doc-page__summary">
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in LIVE_ANCHOR_AUDIT_BACKGROUND" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in LIVE_ANCHOR_AUDIT_GOALS" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <div class="wf-table-wrap">
            <table class="wf-table">
              ...
              <tbody>
                <tr v-for="row in LIVE_ANCHOR_AUDIT_FEATURE_LIST" :key="row.id">
      ...
      <div v-else class="wf-doc-page__features">
        <article
          v-for="row in LIVE_ANCHOR_AUDIT_FEATURE_LIST"
          :key="row.id"
