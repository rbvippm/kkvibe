...
import {
  REBATE_AGENT_H5_CONFIG_BACKGROUND,
  REBATE_AGENT_H5_CONFIG_FEATURE_LIST,
  REBATE_AGENT_H5_CONFIG_GOALS,
  REBATE_AGENT_H5_CONFIG_META,
} from '../../constants/rebateAgentH5ConfigSpec'
  ...
  <div class="pc-wireframe-page wf-doc-page">
    ...
    <section class="wf-block">
      <header class="wf-doc-page__header">
        <p class="wf-doc-page__module">所属模块：{{ REBATE_AGENT_H5_CONFIG_META.module }}</p>
        <h2 class="wf-doc-page__title">需求名称：{{ REBATE_AGENT_H5_CONFIG_META.title }}</h2>
        <p class="wf-doc-page__meta">
          PRD 版本：{{ REBATE_AGENT_H5_CONFIG_META.prdVersion }} · 最近更新：{{
            REBATE_AGENT_H5_CONFIG_META.updatedAt
          }}
        </p>
        <p class="wf-doc-page__hint">{{ PRD_DIMENSION_HINT }}</p>
        <RouterLink :to="{ name: 'pc-rebate-agent-h5-config' }" class="wf-doc-page__back">
          ← 返回返佣代理H5配置
        </RouterLink>
      </header>
      ...
      <div v-if="activeTab === 'summary'" class="wf-doc-page__summary">
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in REBATE_AGENT_H5_CONFIG_BACKGROUND" :key="index">
              {{ item }}
            </li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in REBATE_AGENT_H5_CONFIG_GOALS" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <div class="wf-table-wrap">
            <table class="wf-table">
              ...
              <tbody>
                <tr v-for="row in REBATE_AGENT_H5_CONFIG_FEATURE_LIST" :key="row.id">
      ...
      <div v-else class="wf-doc-page__features">
        <article
          v-for="row in REBATE_AGENT_H5_CONFIG_FEATURE_LIST"
          :key="row.id"
