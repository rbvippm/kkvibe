...
import {
  LIVE_BROADCAST_LIST_BACKGROUND,
  LIVE_BROADCAST_LIST_FEATURE_LIST,
  LIVE_BROADCAST_LIST_GOALS,
  LIVE_BROADCAST_LIST_META,
} from '../../constants/liveBroadcastListSpec'
  ...
  <div class="pc-wireframe-page wf-doc-page">
    ...
    <section class="wf-block">
      <header class="wf-doc-page__header">
        <p class="wf-doc-page__module">所属模块：{{ LIVE_BROADCAST_LIST_META.module }}</p>
        <h2 class="wf-doc-page__title">需求名称：{{ LIVE_BROADCAST_LIST_META.title }}</h2>
        <p class="wf-doc-page__meta">
          PRD 版本：{{ LIVE_BROADCAST_LIST_META.prdVersion }} · 最近更新：{{ LIVE_BROADCAST_LIST_META.updatedAt }}
        </p>
        <p class="wf-doc-page__hint">{{ PRD_DIMENSION_HINT }}</p>
        <RouterLink :to="{ name: 'pc-live-broadcast-list' }" class="wf-doc-page__back">
          ← 返回直播列表
        </RouterLink>
      </header>
      ...
      <div v-if="activeTab === 'summary'" class="wf-doc-page__summary">
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in LIVE_BROADCAST_LIST_BACKGROUND" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in LIVE_BROADCAST_LIST_GOALS" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <div class="wf-table-wrap">
            <table class="wf-table">
              ...
              <tbody>
                <tr v-for="row in LIVE_BROADCAST_LIST_FEATURE_LIST" :key="row.id">
      ...
      <div v-else class="wf-doc-page__features">
        <article
          v-for="row in LIVE_BROADCAST_LIST_FEATURE_LIST"
          :key="row.id"
