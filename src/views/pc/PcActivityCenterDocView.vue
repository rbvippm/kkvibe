...
import {
  ACTIVITY_CENTER_BACKGROUND,
  ACTIVITY_CENTER_FEATURE_LIST,
  ACTIVITY_CENTER_GOALS,
  ACTIVITY_CENTER_META,
} from '../../constants/activityCenterSpec'
  ...
  <div class="pc-wireframe-page wf-doc-page">
    ...
    <section class="wf-block">
      <header class="wf-doc-page__header">
        <p class="wf-doc-page__module">所属模块：{{ ACTIVITY_CENTER_META.module }}</p>
        <h2 class="wf-doc-page__title">需求名称：{{ ACTIVITY_CENTER_META.title }}</h2>
        <p class="wf-doc-page__meta">
          PRD 版本：{{ ACTIVITY_CENTER_META.prdVersion }} · 最近更新：{{ ACTIVITY_CENTER_META.updatedAt }}
        </p>
        <p class="wf-doc-page__hint">{{ PRD_DIMENSION_HINT }}</p>
        <RouterLink :to="{ name: 'pc-activity-center' }" class="wf-doc-page__back">
          ← 返回活动中心
        </RouterLink>
      </header>
      ...
      <div v-if="activeTab === 'summary'" class="wf-doc-page__summary">
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in ACTIVITY_CENTER_BACKGROUND" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in ACTIVITY_CENTER_GOALS" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <div class="wf-table-wrap">
            <table class="wf-table">
              ...
              <tbody>
                <tr v-for="row in ACTIVITY_CENTER_FEATURE_LIST" :key="row.id">
      ...
      <div v-else class="wf-doc-page__features">
        <article
          v-for="row in ACTIVITY_CENTER_FEATURE_LIST"
          :key="row.id"
