...
import {
  STICKER_PACK_MANAGE_BACKGROUND,
  STICKER_PACK_MANAGE_FEATURE_LIST,
  STICKER_PACK_MANAGE_GOALS,
  STICKER_PACK_MANAGE_META,
} from '../../constants/stickerPackManageSpec'
  ...
  <div class="pc-wireframe-page wf-doc-page">
    ...
    <section class="wf-block">
      <header class="wf-doc-page__header">
        <p class="wf-doc-page__module">所属模块：{{ STICKER_PACK_MANAGE_META.module }}</p>
        <h2 class="wf-doc-page__title">需求名称：{{ STICKER_PACK_MANAGE_META.title }}</h2>
        <p class="wf-doc-page__meta">
          PRD 版本：{{ STICKER_PACK_MANAGE_META.prdVersion }} · 最近更新：{{ STICKER_PACK_MANAGE_META.updatedAt }}
        </p>
        <p class="wf-doc-page__hint">{{ PRD_DIMENSION_HINT }}</p>
        <RouterLink :to="{ name: 'pc-sticker-pack-manage' }" class="wf-doc-page__back">
          ← 返回贴图包管理
        </RouterLink>
      </header>
      ...
      <div v-if="activeTab === 'summary'" class="wf-doc-page__summary">
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in STICKER_PACK_MANAGE_BACKGROUND" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in STICKER_PACK_MANAGE_GOALS" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <div class="wf-table-wrap">
            <table class="wf-table">
              ...
              <tbody>
                <tr v-for="row in STICKER_PACK_MANAGE_FEATURE_LIST" :key="row.id">
      ...
      <div v-else class="wf-doc-page__features">
        <article
          v-for="row in STICKER_PACK_MANAGE_FEATURE_LIST"
          :key="row.id"
