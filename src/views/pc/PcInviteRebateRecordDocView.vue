...
import {
  INVITE_REBATE_RECORD_BACKGROUND,
  INVITE_REBATE_RECORD_FEATURE_LIST,
  INVITE_REBATE_RECORD_GOALS,
  INVITE_REBATE_RECORD_META,
  ...
  <div class="pc-wireframe-page wf-doc-page">
    ...
    <section class="wf-block">
      <header class="wf-doc-page__header">
        <p class="wf-doc-page__module">所属模块：{{ INVITE_REBATE_RECORD_META.module }}</p>
        <h2 class="wf-doc-page__title">需求名称：{{ INVITE_REBATE_RECORD_META.title }}</h2>
        <p class="wf-doc-page__meta">
          PRD 版本：{{ INVITE_REBATE_RECORD_META.prdVersion }} · 最近更新：{{ INVITE_REBATE_RECORD_META.updatedAt }}
        </p>
        <p class="wf-doc-page__hint">{{ PRD_DIMENSION_HINT }}</p>
        <RouterLink :to="{ name: 'pc-invite-rebate-records' }" class="wf-doc-page__back">
          ← 返回活动明细
        </RouterLink>
      </header>
      ...
      <div v-if="activeTab === 'summary'" class="wf-doc-page__summary">
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in INVITE_REBATE_RECORD_BACKGROUND" :key="index">{{ item }}</li>
          </ul>
        </section>
        <section class="wf-doc-page__section">
          <h3 class="wf-doc-page__section-title">2. 需求目标</h3>
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in INVITE_REBATE_RECORD_GOALS" :key="index">{{ item }}</li>
          </ul>
        </section>
        <section class="wf-doc-page__section">
          ...
          <div class="wf-table-wrap">
            <table class="wf-table">
              ...
              <tbody>
                <tr v-for="row in INVITE_REBATE_RECORD_FEATURE_LIST" :key="row.id">
      ...
      <div v-else class="wf-doc-page__features">
        <article
          v-for="row in INVITE_REBATE_RECORD_FEATURE_LIST"
          :key="row.id"
