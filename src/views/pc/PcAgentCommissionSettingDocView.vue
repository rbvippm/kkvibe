...
import {
  AGENT_COMMISSION_SETTING_BACKGROUND,
  AGENT_COMMISSION_SETTING_FEATURE_LIST,
  AGENT_COMMISSION_SETTING_GOALS,
  AGENT_COMMISSION_SETTING_META,
} from '../../constants/agentCommissionSettingSpec'
  ...
  <div class="pc-wireframe-page wf-doc-page">
    ...
    <section class="wf-block">
      <header class="wf-doc-page__header">
        <p class="wf-doc-page__module">所属模块：{{ AGENT_COMMISSION_SETTING_META.module }}</p>
        <h2 class="wf-doc-page__title">需求名称：{{ AGENT_COMMISSION_SETTING_META.title }}</h2>
        <p class="wf-doc-page__meta">
          PRD 版本：{{ AGENT_COMMISSION_SETTING_META.prdVersion }} · 最近更新：{{
            AGENT_COMMISSION_SETTING_META.updatedAt
          }}
        </p>
        <p class="wf-doc-page__hint">{{ PRD_DIMENSION_HINT }}</p>
        <RouterLink :to="{ name: 'pc-agent-commission-setting' }" class="wf-doc-page__back">
          ← 返回返佣金设置
        </RouterLink>
      </header>
      ...
      <div v-if="activeTab === 'summary'" class="wf-doc-page__summary">
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in AGENT_COMMISSION_SETTING_BACKGROUND" :key="index">
              {{ item }}
            </li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <ul class="wf-doc-page__list">
            <li v-for="(item, index) in AGENT_COMMISSION_SETTING_GOALS" :key="index">{{ item }}</li>
          </ul>
        </section>
        ...
        <section class="wf-doc-page__section">
          ...
          <div class="wf-table-wrap">
            <table class="wf-table">
              ...
              <tbody>
                <tr v-for="row in AGENT_COMMISSION_SETTING_FEATURE_LIST" :key="row.id">
      ...
      <div v-else class="wf-doc-page__features">
        <article
          v-for="row in AGENT_COMMISSION_SETTING_FEATURE_LIST"
          :key="row.id"
