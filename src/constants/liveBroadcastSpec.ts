...
export const LIVE_BROADCAST_FEATURE_LIST: LiveBroadcastFeatureRow[] = [
  {
    ...
    prd: {
      ...
      visualPresentation: '工具栏左侧文案「当前直播间：{名称}（{roomId}）」，名称加粗。',
  ...
  },
  {
    ...
    prd: {
      ...
      visualPresentation: '工具栏右侧蓝色链接「禁言列表（N）」；N 为 muted=true 的记录总数；旁侧「注」标注。',
  ...
  },
  {
    ...
    prd: {
      ...
      visualPresentation: '16:9 黑色背景区域，居中灰色占位文案「直播画面」；最小高度 280px。',
  ...
  },
  {
    ...
    prd: {
      ...
      visualPresentation:
        '标签「直播提醒：」+ 输入框（placeholder「请输入提醒内容」）+ 主色「发送」按钮；下方红色说明文案；旁侧「注」标注。',
  ...
  },
  {
    ...
    prd: {
      ...
      visualPresentation:
        '侧栏标题「弹幕消息」+ 点赞/观看统计胶囊；列表项含头像、内容、时间；空态「暂无弹幕消息」；旁侧「注」标注。',
      dataRules:
  ...
  },
  {
    ...
    prd: {
      ...
      visualPresentation: '整行 opacity 降低；内容下方红色小标签「已禁言」。',
      dataRules:
  ...
  },
  {
    ...
    prd: {
      ...
      visualPresentation: '固定定位白底菜单，标题为用户名（系统消息带「系统」标签）；菜单项「禁言」；旁侧「注」标注。',
  ...
  },
  {
    ...
    prd: {
      ...
      visualPresentation: '单选组：房间禁言 / 全局禁言；展示目标用户与触发弹幕摘要；旁侧「注」标注。',
      dataRules:
        '默认值：房间禁言；muteSource 记为「主播」；muteType 写入禁言记录并与禁言列表同步。',
  ...
  },
  {
    ...
    prd: {
      ...
      visualPresentation: '必填标签「禁言原因」+ 输入框 maxlength=50；底部「取消」「确定禁言」；校验提示 wf-modal__hint；旁侧「注」标注。',
      dataRules:
...
/**
 * 页面「注」标记编号 · 与 LIVE_BROADCAST_FEATURE_LIST.id 对齐
 * 无页面标注的功能（如 #1、#3、#6）不在此列出
 */
...
export const LIVE_BROADCAST_DANMAKU_LIST_SPEC = [
  ...
  '点击或右键弹幕可打开操作菜单；已禁言用户消息带「已禁言」标签且整行半透明。',
] as const
