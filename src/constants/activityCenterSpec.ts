...
export const ACTIVITY_CENTER_BACKGROUND = [
  ...
  '「邀请好友充值返利（VIP阶梯自动版）」按币种配置金额条件并向邀请人按币种派发返利；仅双方均为普通会员时计奖，成为代理则取消资格。',
...
export const ACTIVITY_CENTER_GOALS = [
  ...
  '编辑/详情弹窗按币种配置：邀请人（历史累计存款 / 每日最低存款 / 绑定手机号）、被邀请人（历史累计存款 / 每日最低存款）、VIP 日返利上限。',
  '明确结算与上限：隔天 GMT+8 12:00 派发昨天返利（昨天双方每日最低存款与历史累计存款均须达标）；上限按计算日 23:59:59 被邀请人 VIP 快照，落库金额同步落库上限，超出各上限之和则扣减后派发。',
] as const
...
export const ACTIVITY_CENTER_FEATURE_LIST: ActivityCenterFeatureRow[] = [
  ...
  },
  {
    ...
    prd: {
      functionalLogic:
        '当活动类型为「邀请好友充值返利（VIP阶梯自动版）」时，弹窗展示专属规则区。邀请人条件：历史累计存款、每日最低存款、绑定手机号。被邀请人条件：历史累计存款、每日最低存款。活动条件按币种区分金额，并按币种向邀请人返利。仅双方均为普通会员时计算返利，任一方成为代理则取消返利资格。结算：隔天业务时区 GMT+8 12:00 派发昨天返利，且昨天双方每日最低存款均须达标，并且历史累计存款也要达标。返利上限取返利计算日 23:59:59 被邀请人 VIP 等级；落库当日返利金额时同步落库当日上限；若当日应发合计超过各被邀请人上限之和，扣减超出后再派发。',
      interactiveBehavior:
        '选择该活动类型 -> 展开规则区；勾选币种后生成对应配置，多币种 Tab 切换。可新增/删除 VIP 阶梯行。点击「保存」执行校验，失败在弹窗内提示；成功关闭弹窗并刷新列表。详情态只读，摘要区说明资格、结算与上限口径。',
      visualPresentation:
        '弹窗标题旁「注2」。规则区顶部说明资格 / 结算 / 上限规则。分组：邀请人条件、被邀请人条件、VIP 日返利上限表。金额输入后缀展示当前配置币种。',
      dataRules:
        '①按币种独立配置金额门槛与 VIP 日上限，返利按该币种派发。②邀请人：绑定手机号（是/否）；为「是」时区号可多选且至少 1 个；历史累计存款、每日最低存款。③被邀请人：历史累计存款、每日最低存款。④VIP 阶梯至少 1 行（VIP0～9）。⑤结算日校验「昨天」双方每日最低存款，且双方历史累计存款也须达标。⑥上限快照时点：返利计算日 23:59:59 被邀请人 VIP；派发时当日应发 ≤ 各被邀请人当日上限之和。',
      exceptions:
        '未勾选币种 / 须绑手机却未选区号 / VIP 阶梯为空 / 金额为负 / VIP 区间不合法 -> 中文校验提示，不关闭弹窗。任一方为代理 -> 不计奖、不展示移动端返利数据。',
      routing:
        '保存成功关闭弹窗并更新列表；无独立子路由。移动端返利展示与「双方均为会员、非代理」资格及代理邀请状态联动。',
    },
  },
]
