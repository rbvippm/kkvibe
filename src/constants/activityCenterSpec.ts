...
export const ACTIVITY_CENTER_BACKGROUND = [
  ...
  '「邀请好友充值返利（VIP阶梯自动版）」按币种配置邀请人/被邀请人门槛、邀请人返利比例、返利提现流水倍数与 VIP 日上限阶梯，并向邀请人按币种派发返利；仅双方均为普通会员时计奖，成为代理则取消资格。',
...
export const ACTIVITY_CENTER_GOALS = [
  ...
  '编辑/详情弹窗按币种配置：邀请人（历史累计存款 / 每日最低存款 / 绑定手机号 / 返利比例 / 返利提现流水倍数）、被邀请人（历史累计存款 / 每日最低存款）、VIP 日返利上限阶梯（注2）。',
  '明确计算、结算与上限：应发返利 = 被邀请人当天存款金额 × 邀请人返利比例；派发时按返利提现流水倍数增加邀请人提现流水（0=无要求）；隔天 GMT+8 12:00 派发昨天返利（昨天双方每日最低存款与历史累计存款均须达标）；上限按计算日 23:59:59 被邀请人 VIP 快照，落库金额同步落库上限，超出各上限之和则扣减后派发。',
] as const
...
export const ACTIVITY_CENTER_FEATURE_LIST: ActivityCenterFeatureRow[] = [
  ...
  },
  {
    ...
    prd: {
      functionalLogic:
        '当活动类型为「邀请好友充值返利（VIP阶梯自动版）」时，弹窗展示专属规则区。邀请人条件：历史累计存款、每日最低存款、绑定手机号、返利比例、返利提现流水倍数。被邀请人条件：历史累计存款、每日最低存款。活动条件按币种区分金额，并按币种向邀请人返利。仅双方均为普通会员时计算返利，任一方成为代理则取消返利资格。结算：隔天业务时区 GMT+8 12:00 派发昨天返利，且昨天双方每日最低存款均须达标，并且历史累计存款也要达标。应发返利 = 被邀请人当天存款金额 × 邀请人返利比例。派发时按「返利金额 × 返利提现流水倍数」增加邀请人提现流水要求，倍数为 0 则不加流水。返利上限取返利计算日 23:59:59 被邀请人 VIP 等级；落库当日返利金额时同步落库当日上限；若当日应发合计超过各被邀请人上限之和，扣减超出后再派发。',
      interactiveBehavior:
        '选择该活动类型 → 展开规则区；勾选币种后生成对应配置，多币种 Tab 切换。邀请人条件内配置返利比例与返利提现流水倍数；VIP 阶梯表可新增/删除行，每行设置方式（单个/区间/及以上）、VIP 等级、每日返利最高上限。点击「保存」执行校验，失败在弹窗内提示；成功关闭弹窗并刷新列表。详情态只读，摘要区说明资格、结算、计算与上限口径。',
      visualPresentation:
        '弹窗标题旁「注2」。规则区顶部说明资格 / 结算 / 计算 / 上限规则。分组：邀请人条件（含返利比例 %、返利提现流水倍数「倍」）、被邀请人条件、被邀请人 VIP 对应 · 邀请人日返利上限表（列：设置方式、VIP 等级、每日返利最高上限、操作）。金额输入后缀展示当前配置币种。',
      dataRules:
        '①按币种独立配置金额门槛、返利比例、返利提现流水倍数与 VIP 日上限，返利按该币种派发。②邀请人：绑定手机号（是/否）；为「是」时区号可多选且至少 1 个；历史累计存款、每日最低存款、返利比例（0～100%）、返利提现流水倍数（整数 ≥0，0=无提现流水要求）。③被邀请人：历史累计存款、每日最低存款。④VIP 阶梯至少 1 行（VIP0～9），每行配置每日返利最高上限；应发返利 = 被邀请人当天存款金额 × 邀请人返利比例。⑤结算日校验「昨天」双方每日最低存款，且双方历史累计存款也须达标。⑥上限快照时点：返利计算日 23:59:59 被邀请人 VIP；派发时当日应发 ≤ 各被邀请人当日上限之和。⑦Mock 默认返利比例 1%、返利提现流水倍数 1。',
      exceptions:
        '未勾选币种 / 须绑手机却未选区号 / VIP 阶梯为空 / 金额为负 / 返利比例不在 0～100 / 返利提现流水倍数非整数或小于 0 / VIP 区间不合法 → 中文校验提示，不关闭弹窗。任一方为代理 → 不计奖、不展示移动端返利数据。',
      routing:
...
export const ACTIVITY_CENTER_INVITE_REBATE_CONFIG_SPEC = [
  '邀请人条件：历史累计存款、每日最低存款、绑定手机号、返利比例、返利提现流水倍数；选「是」时须复选至少一个区号。',
  '返利提现流水倍数：整数，最小 0；0 表示该币种返利无提现流水要求；>0 时按「返利金额 × 倍数」增加邀请人提现流水。',
  '被邀请人条件：历史累计存款、每日最低存款。',
  '按币种配置金额条件、返利比例与流水倍数，并按币种向邀请人返利；多币种用 Tab 分别配置。',
