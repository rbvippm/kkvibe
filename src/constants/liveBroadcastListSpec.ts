...
export const LIVE_BROADCAST_LIST_GOALS = [
  '筛选和列表都能区分「正在直播」「直播预告」。',
  '人数、预约、热度、点赞四列统一展示总数 / 基准 / 实际，口径与主播列表生效配置对齐。',
]
...
export const LIVE_BROADCAST_LIST_FEATURE_LIST: LiveBroadcastListFeatureRow[] = [
  ...
  },
  {
    ...
    prd: {
      ...
      dataRules:
        '非负整数。能对上主播列表的主播（如小夜不困）基准与主播列表生效基准人数一致，原型 280。实际为叠加后的展示人数，原型小夜不困 346。总数 626。直播预告场次仍展示当前预热人数，不为空。',
      exceptions: '对不上主播列表时基准待接口确认，原型仍给出 Mock 数。无数据行不展示 0 以外的空串。',
  ...
  },
  {
    ...
    prd: {
      ...
      dataRules:
        '非负整数。小夜不困原型基准 60、实际 78、总数 138，与主播列表基础预约对齐。游客预约不计入（与主播列表规则一致）。',
  ...
  },
  {
    ...
    prd: {
      ...
      dataRules:
        '非负整数。小夜不困原型基准 180、实际 412、总数 592，基准与主播列表本场点赞对齐。游客点赞不叠加。',
