...
export const AGENT_REPORT_GAME_STATS_SPEC: MobilePrdSpec = {
  ...
  sections: buildMobilePrdSections({
    ...
    interaction: [
      ...
      '点筛选条左侧日期整块（文案或日历）打开底部「筛选时间」（标题下两行三列：今天 / 昨天 / 本周 / 上周 / 本月 / 上月，点选回填起止日；也可滚轮自选，改滚轮后弹层内快捷高亮取消），确定后回显起止日并取消页内快捷高亮；点页内快捷时间回填对应区间。点右侧币种框打开底部「选择币种」Sheet，与盈亏/佣金、概况共用。',
    ],
    visual: [
      '页头仅「佣金/盈亏 + 游戏」Tab；币种不在页头。筛选条对齐盈亏/佣金：左右两框等高 32、间距 8、各占一半（12 字号；左日期 + 橙线框日历 16，右 20 币种图标 + 文案 + 24 下拉热区）；下一行「今天/昨天/本月/上月」四等分高 30（选中橙描边浅橙底）。占成与返佣游戏页同一套筛选。选择币种 Sheet 与概况共用，选中文字橙色；入口回显当前图标。币种为代理端全局态。',
    ...
    exception: [
      ...
      '本标注仅返佣身份展示；占成游戏报表明细标题对齐代理详情，带「（实占）」后缀（如「全部（实占）」）；公式为「实占游戏输赢 − 实占退水 − 实占VIP退水 − 场馆费」，明细须含场馆费、不含代理赚水。',
    ],
    routing: [
