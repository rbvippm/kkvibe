...
1. **功能逻辑** `logic`：模块做什么、业务目标、主链路。
2. **交互行为** `interaction`：用户动作 -> 系统反馈 / 状态变化。
3. **视觉表现** `visual`：布局、颜色、组件状态、动效；仅写业务 UI，禁止写「注N」标注入口或 PRD 浮层本身。
4. **数据规则** `data`：格式、枚举、限制、默认值；使用界面中文名，禁止代码字段名和英文枚举。
5. **异常与边界** `exception`：错误文案、空状态、超时、权限、禁用态。
6. **关联与跳转** `routing`：入口、出口、路由名、页面勾连。
