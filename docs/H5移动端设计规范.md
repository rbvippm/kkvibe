...
适用于 H5 / 代理端移动原型的「标注」。移动端标注与 PC 后台 `WfSpecAnnot` 分离，不得混用组件或数据结构。后续移动端 PRD 不增加注N 数量，放到对应的功能位置即可，名字默认叫标注。
...
- **三、视觉表现** `visual`：布局、颜色、组件状态、动效；仅写业务 UI，禁止写「标注」标注入口或 PRD 浮层本身。
...
- 触发器名字默认叫「标注」，不递增「注N」，不维护 `{MODULE}_SPEC_ANNOT_NO`。
- 把标注放到对应的功能位置；同一页多条「标注」靠位置区分，不靠数字。
- 一条「标注」只描述一个明确功能或流程。
...
- **视觉表现** 只写业务 UI，禁止描述「标注」「需求说明」「PRD 浮层」「标注触发」等标注组件自身。
...
- [ ] 如页面有「标注」，已同步维护 `MobilePrdSpec`，且放在对应功能旁。
